...
Real verified edition covers replace the reference's illustrated volumes. Only the three real published entries appear; no mockup books or unsupported synopses are invented. The first populated shelf opens by default rather than showing an empty Reading shelf. Existing review links are retained. The shared green, timber-framed site navigation remains above the window in normal document flow. Timber, leaf ornaments, and parchment use production CSS approximations instead of raster interface artwork.
