...
async function navigationAppearance(locator: Locator) {
  return locator.evaluate((element) => {
    ...
    return [
      element,
      element.querySelector('.frame-surface')!,
      ...element.querySelectorAll('.site-navigation__link'),
    ].map((node) => {
      ...
      return {
        ...
        color: node.matches('.site-navigation__link') ? style.color : undefined,
        ...Object.fromEntries(properties.map((key) => [key, style[key]])),
      };
    });
  });
}
...
for (const width of [320, 390, 760, 1024, 1586]) {
  test(`page chrome is consistent and specimens fit at ${width}px`, async ({
    page,
    baseURL,
  }, testInfo) => {
    ...
    for (const route of [
      ...
      const geometry = await page.evaluate(() => {
        ...
        if (Math.abs(content.top - bounds.bottom - gap) > 1)
          throw new Error('Navigation must sit above content with the shared gap');
        return {
          x: bounds.x,
          y: bounds.y,
          width: bounds.width,
    ...
    await expect(page.locator('.navigation-specimen a, .navigation-specimen button')).toHaveCount(
      0,
    );
    await expect(page.locator('.navigation-specimen [aria-current="page"]')).toHaveCount(1);
    const specimenNavigation = await navigationAppearance(
      page.locator('.navigation-specimen .site-navigation'),
    );
    expect(projectNavigation).toBeDefined();
    expect(specimenNavigation.map(({ width, height, ...styles }) => styles)).toEqual(
      projectNavigation!.map(({ width, height, ...styles }) => styles),
    );
    await expect(page.getByRole('link', { name: /Return to the Personal Log/ })).toHaveCount(0);
    await expect(page.locator('.book-tile-specimens .codex-tile')).toHaveCount(2);
    const cvSpecimen = page
      .locator('.component-grid > article')
      .filter({ has: page.getByRole('heading', { name: 'CTA icons', exact: true }) });
    await expect(cvSpecimen.locator('.pixel-icon')).toHaveCount(3);
    await expect(
      cvSpecimen.getByRole('link', { name: 'LinkedIn (opens in a new tab)' }),
    ).toHaveAttribute('href', 'https://www.linkedin.com/in/bernardosevero/');
    await expect(
      cvSpecimen.getByRole('link', { name: 'GitHub (opens in a new tab)' }),
    ).toHaveAttribute('href', 'https://github.com/bernardosevero');
    await expect(cvSpecimen.getByRole('link', { name: 'Download CV (PDF)' })).toHaveCount(
      cvAsset ? 1 : 0,
    );
    ...
    const specimenProblems = await page
      ...
      .evaluateAll((specimens) =>
        specimens.flatMap((specimen, index) => {
          ...
          const overlap = children.some(
            (child, i) => i > 0 && child.top < children[i - 1].bottom - 1,
          );
          return outside || overlap || specimen.scrollWidth > specimen.clientWidth + 1
            ? [index]
            : [];
        }),
      );
    expect(
      specimenProblems,
      'Specimens must contain examples without overlapping labels or descriptions',
    ).toEqual([]);
    expect(await page.evaluate(() => document.documentElement.scrollWidth <= innerWidth)).toBe(
      true,
    );
