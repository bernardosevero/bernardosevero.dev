...
const byNotionId = new Map(
  books.filter((book) => book.notionId).map((book) => [book.notionId, book]),
);
...
const knownEnglishMetadata = new Map([
  ...
  [
    '3d474cb445b881199053ee7d6730761a',
...
const normalizeIsbn = (value) =>
  value == null ? undefined : String(value).replace(/[\s-]/g, '').toUpperCase();
const reviewUpdates = [];
...
for (const source of incoming) {
  ...
  if (!title || !author)
    throw new Error(`English title and author are required for ${source.title}.`);
  ...
  else book.rating = source.rating;
  if (!source.finishedAt) delete book.finishedAt;
  else book.finishedAt = source.finishedAt;
  ...
  if (existing) Object.assign(existing, book);
    ...
    books.push(book);
    byId.set(id, book);
    byNotionId.set(source.notionId, book);
  }
...
for (const review of reviewUpdates) {
  ...
  let current;
  try {
    current = await readFile(reviewPath, 'utf8');
  } catch (error) {
    if (error.code !== 'ENOENT') throw error;
  }
  if (current !== undefined && !review.existing)
    throw new Error(`Review file already exists for new book: ${review.id}`);
...
console.log(
  `Imported ${incoming.length} book${incoming.length === 1 ? '' : 's'} without deleting local entries.`,
);
